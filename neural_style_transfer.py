...
def build_loss(neural_net, optimizing_img, target_representations, content_feature_maps_index, style_feature_maps_indices, config):
    ...
    content_loss = F.mse_loss(target_content_representation, current_content_representation)

    style_loss = 0.0
    current_style_representation = [utils.gram_matrix(x) for cnt, x in enumerate(current_set_of_feature_maps) if cnt in style_feature_maps_indices]
    for gram_gt, gram_hat in zip(target_style_representation, current_style_representation):
        style_loss += F.mse_loss(gram_gt[0], gram_hat[0])
...
def neural_style_transfer(config):
    ...
    num_of_iterations = config['steps']
    ...
    if config['optimizer'] == 'lbfgs':
        ...
        optimizer = LBFGS((optimizing_img,), max_iter=num_of_iterations)
        cnt = 0
...
if __name__ == "__main__":
    ...
    output_img_dir = os.path.join(default_resource_dir, 'outputs')
    ...
    parser.add_argument("--optimizer", type=str, choices=['lbfgs', 'adam', 'adamw'], default='lbfgs')
    parser.add_argument("--lr", type=float, help="learning rate for adam and adamw", default=1e1)
    parser.add_argument("--model", type=str, choices=['vgg16', 'vgg19', 'resnet'], default='vgg19')
    parser.add_argument("--init_method", type=str, choices=['random', 'content', 'style'], default='content')
    parser.add_argument("--steps", type=int, help="total steps", default=1000)
    ...
    optimization_config['saving_freq'] = int(args.steps * 0.01)
